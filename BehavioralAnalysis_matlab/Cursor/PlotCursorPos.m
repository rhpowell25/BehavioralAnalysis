function [avg_z_cursor] = PlotCursorPos(xds, event, unit_name, cursor_pos_YLims, Plot_Figs, Save_Figs)
...
for jj = 1:num_dirs
    ...
    %% Extracting cursor position during successful trials

    % Cursor position measured during each successful trial 
    ...
    all_z_cursor_p = zeros(length(z_cursor_p{1,1}), length(z_cursor_p));
    for ii = 1:length(z_cursor_p)
        all_z_cursor_p(:,ii) = z_cursor_p{ii,1}(:,1);
    end

    %% Average the recomposed cursor positions info
    avg_z_cursor{jj,1} = zeros(length(all_z_cursor_p),1);
    for ii = 1:length(avg_z_cursor{jj,1})
        avg_z_cursor{jj,1}(ii) = mean(all_z_cursor_p(ii,:));
    end

    %% Find the standard dev of the recomposed cursor position info

    std_z_cursor = zeros(length(all_z_cursor_p),1);
    for ii = 1:length(avg_z_cursor{jj,1})
        std_z_cursor(ii) = std(all_z_cursor_p(ii,:));
    end
    ...
    if isequal(Plot_Figs, 1)
        ...
        % Average
        plot(absolute_timing, avg_z_cursor{jj,1}, 'k', 'LineWidth', 2)
        % Standard Dev
        plot(absolute_timing, avg_z_cursor{jj,1} + std_z_cursor, ...
